...
char currentPassword[PassLength] = {'0', '0', '0', '0'}; // default password before setting
...
int unlockMode(){
  ...
  for(int i = 0; i < PassLength; i++) {
    while(!(result = myAwesomePad.getKey())) {
      //  wait indefinately for keypad input of any kind
    }
  ...
  }  // done after 4 characters (PassLength) are accepted
...
void setup() {
  ...
  Serial.println("Press # to access the system with an existing one.");
...
void loop() {
  ...
  if (result == '*') {   // choice to change a password
    int access = unlockMode();

  if (access < 0) {
    Serial.println("Access Denied.  Cannot change password withoug knowing the previous password");
  }
  else {
    ...
    for(int i = 0; i < PassLength; i++)  {
      while(!(result = myAwesomePad.getKey())) {
        // wait indefinately for a keypad input of any kind
      }
    ...
    Serial.println(""); 
    Serial.println("Password Successfully Chenged!");
            
    }
  }

  else if (result == "#")  {   // choice to access the system wiith current password
    int access = unlockMode();
    if(access < 0)  {
